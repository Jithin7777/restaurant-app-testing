    ...
    setCartItems((prev) => {
      return prev.map((ci) => {
        const isSameItem =
          ...
          JSON.stringify((ci.extras ?? []).map((e) => e.item.id).sort()) ===
            JSON.stringify(
              (mainItem.extras ?? []).map((e) => e.item.id).sort(),
            );

        if (!isSameItem) return ci;

        const updatedExtras = (ci.extras || []).map((e) =>
          e.item.id === extra.id ? { ...e, quantity: e.quantity + 1 } : e,
        );
    ...
    setCartItems((prev) => {
      const existingIndex = prev.findIndex((ci) => {
        ...
        const sameExtras =
          ...
          ci.extras?.every((e) =>
            extras.find(
              (ex) => ex.item.id === e.item.id && ex.quantity === e.quantity,
            ),
          );
    ...
    return cartItems.reduce((total, cartItem) => {
      ...
      const extrasTotal =
        cartItem.extras?.reduce(
          (acc, extra) => acc + parseFloat(extra.item?.price) * extra.quantity,
          0,
        ) || 0;
